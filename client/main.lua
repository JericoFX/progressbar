...
function Process(action, start, tick, finish)
	ActionStart()
    Action = action
	  TriggerServerEvent("OpenInventory",false) --disable on any progressbar
    ...
    if not IsEntityDead(ped) or Action.useWhileDead then
        if not isDoingAction then
			...
			TriggerServerEvent("OpenInventory",true) --Enable Inventory on Cancel
                    end

                    if IsEntityDead(ped) and not Action.useWhileDead then
                        TriggerEvent("progressbar:client:cancel")
			TriggerServerEvent("OpenInventory",false) --disable when dead
                    end
                end
                if finish ~= nil then
                    finish(wasCancelled)
